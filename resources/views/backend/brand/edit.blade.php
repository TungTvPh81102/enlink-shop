            ...
            <div class="card-body">
                ...
                <form action="{{ route('admin.brands.update', $brand->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group ">
                        ...
                        <input type="text" class="form-control mb-3" name="name" value="{{ old('name', $brand->name) }}"
                    ...
                    <div class="form-group ">
                        <label for="formGroupExampleInput">Preview</label>
                        <br>
                        <img src="{{ Storage::url($brand->image) }}" style="width: 100px; height: 100px" alt="">
                    </div>
                    <div class="form-group">
                        <label for="formGroupExampleInput2">Trạng thái</label>
                        <select name="status" id="" class="form-control mb-3">
                            <option value="">--- Chọn ---</option>
                            <option {{ old('status',$brand->status) ? 'selected' : '' }} value="1">Active</option>
                            <option {{ !old('status',$brand->status) ? 'selected' : '' }} value="0">Inactive</option>
                        </select>
                        @error('status')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <a href="{{ route('admin.brands.index') }}" class="btn btn-secondary">Danh sách</a>
                    </div>
                </form>
            </div>
        </div>
